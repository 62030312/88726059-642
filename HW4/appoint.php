...
<body>
    <div class="container">
        <h1>จัดการคำสั่งแต่งตั้ง| <a href='mainpage.php'><span class='glyphicon glyphicon-home'></span></a> | <a href='addappoint.php'><span class='glyphicon glyphicon-plus'></span></a></h1>
        <form action="#" method="post">
            <input type="text" name="kw" placeholder="ชื่อหรือเลขที่คำสั่ง" value="">
            <input type="submit">
        </form>
        ...
        $sql = "SELECT *
                FROM documents
                WHERE concat(id, doc_num, doc_title, doc_start_date	, doc_to_date, doc_status, doc_file_name ) LIKE ? 
                ORDER BY id";
        ...
        if ($result->num_rows == 0) {
            ...
            while($row = $result->fetch_object()){ 
                ...
                $table.= "<td>";
                $table.= "<a href='editappoint.php?id=$row->id'><span class='glyphicon glyphicon-pencil' aria-hidden='true'></span></a>";
                $table.= " | ";
                $table.= "<a href='deleteappoint.php?id=$row->id'><span class='glyphicon glyphicon-trash' aria-hidden='true'></span></a>";
                $table.= "</td>";
                $table.= "</tr>";
            }
